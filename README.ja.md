...
> **署名について**: pdf-lib は保存時にファイル全体を再構築するため、通常の編集では既存の電子署名は必ず無効化されます。`/ByteRange` を含む PDF は既定でエラーとし、`allowBreakingSignatures: true` で破壊的に続行するか、`add_annotation` に限り `preserveSignatures: true`（v0.9.0）で**署名を保持したまま**増分更新で追加できます。実測: 実署名（CMS）PDF への追加後も pdf-verify-mcp は署名 **VALID** + 追記分を合法な増分リビジョン 1 件と報告します。
...
- [ ] Tier C 続き — タグ付き文書の増分更新・他ツールへの展開、`ensure_tagged`、`edit_text`
