...
- 🔴 **`snake_case` の `_` が消えます**（`create_markdown_pdf`・B-17・未修正）。1 行に `_` が偶数個あると、Markdown の強調記号とみなして除去します。`identify_conformance と validate_conformance` → `identifyconformance と validateconformance`。**バッククォートで囲んでも防げず**（コードスパンの復元が `_` の処理より後のため）、**警告も出ません**。関数名・ツール名を含む技術文書では、生成後に読み戻して照合してください。CommonMark は `*` の語中強調は許し `_` は許さない規則を持ちますが、現状の実装は両者を同じに扱っています。
...
> **解決済み**: サブセット名の接頭辞（`ABCDEF+`）は **v0.14.0（W-3）で付与するようになりました**。以前の README にあった「接頭辞を付けない」という制約は解消済みです。
