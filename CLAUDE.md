...
（ADR-11 / `services/incremental.ts`）。対応: `add_annotation`（v0.9.0・タグ無しのみ）、
`set_metadata` / `add_bookmarks`（v0.10.0）。DocMDP の許可レベル検査
（`assertDocMdpAllows`: 注釈 = P=3 のみ / メタデータ・しおり = 全レベル不可）を必ず通すこと。
...
| ファイル | 対象 |
|----------|------|
| `registry.test.ts` | **外部仕様のスナップショット**（18 ツールの名前・必須フィールド・annotations。InMemoryTransport 経由） |
