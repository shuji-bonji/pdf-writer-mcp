...
| 編集 Tier B | `attach_file` / `add_watermark` / `stamp_page_numbers` / `fill_form` / `flatten_form` |
...
| ファイル | 対象 |
|----------|------|
| `registry.test.ts` | **外部仕様のスナップショット**（17 ツールの名前・必須フィールド・annotations。InMemoryTransport 経由） |
