/**
 * Outline (bookmarks) — **旧実装（pdf-lib）**。
 *
 * L4′.2 で COS 版（`outline.ts`）を書いた。`add_bookmarks` が新経路へ移るまで
 * `editor.ts` がこちらを使う。移り終えたらこのファイルは消える
...
// `countBookmarks` は COS 版（`outline.ts`）へ移した —— pdf-lib に触れない数え上げなので
// 両方から使える。
