 ...
 * だから `reserve()` は空の値を置いて番号を押さえる。
 *
 * ⚠️ **閉じたのは片方向だけである**（実測 2026-08-18、素の node）:
 *
 * ```
 * reserve() → 3 / editor.allocate() → 4 / reserve() → 4   ← 重複
 * ```
 *
 * `reserve()` の `#next` は editor が配った番号を見ていない。したがって
 * **「生成パスでは `editor.allocate()` を呼ばない」は依然として約束であり、
 * コードが強制するものではない。** 現状この約束を破る呼び出しは無い —— 実測:
 * `src/` で `.editor` を受けるのは `opened.editor` 67 件（編集パス）と
 * `WriterDocument` 内部の `this.editor` 8 件だけで、`doc.editor` は 0 件。
 * `editor.allocate` を呼ぶ 27 箇所はすべて前者から辿れる。
 * 逆向きも閉じるには `reserve()` が `editor.changed()` を毎回走査することになり、
 * 通らない経路のために採番を O(n) にする。**測って通らないと分かったので足さない。**
 *
